    ...
    <div class="main-container" v-if="!loading">
      <div class="card" @click="$router.push('/read/' + book.id)" v-for='(book) in onlyParsed' :key='"book-" + book.id'>
        ...
        <div class="card-text">
          ...
          <h3 v-if="book.description">{{ book.description && 45 > book.description.length ? book.description : book.description.substring(0,45)+"..."  }}</h3>
          <h2>{{book.author}}</h2>
  ...
  methods: {
    refreshLibrary() {
      ...
      this.axios
          .get('/api/v1/library')
          .then((res) => {
            this.books.count = res.data.count
            this.books.items = res.data.rows
            this.loading = false
          })
          .catch((e) => {
            this.loading = false
